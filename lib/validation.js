...
// tests
	...
	var is = {
		...
		hex_color: function(s) {
			return regTest(s, "hex_color");
		},
...
// built-in validators
	...
	var error_messages = {
		...
		hex_color: "This is not a valid hex value"
	};
...
// field validation
	...
	function validate(field, validators) {
		var value, token, errors, type, master;
				...
				if ($.isFunction(ValidationManager.validators[v])) {
					...
					switch (v) {
						...
						case "date":
							...
							break;
						case "hex_color":
							if (value && !is.hex_color(value)){
								token.addError(error_messages["hex_color"]);
							}
							break;
						default:
							break;
