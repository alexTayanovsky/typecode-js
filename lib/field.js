...
define(['jquery', 'NIseed'], function($) {
	...
	console = NI.fn.get_console(true);
...
// utils, factories, etc
	...
	field.synthesize = function(options) {
		...
		field[o.name] = function(user_options) {
			...
			if (o.base && o.base.prototype && o.base.prototype.options) {
				...
				console.error("invalid base object:");
				console.log(o.base);
			}
			...
			if (this.internal.o.val || this.internal.o.val === "") {
	...
	field.prep_element = function(element, preprocess) {
		...
		is_std = ($.inArray($f[0].nodeName.toLowerCase(), field.co.STANDARD_NODES) > -1);
	...
	field.new_instance = function(options) {
		...
		if (o.options.is_fset) {
			field_info.is_fset = true;
		}
		...
		if (! $.isFunction(field[field_type])) {
			console.error('Field type ' + field_type + 'is not defined');
		}
